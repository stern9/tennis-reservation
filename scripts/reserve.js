...
const ARGS = {
  ...
  watchMode: process.env.WATCH_MODE === 'true' || process.argv.includes('--watch'),
...
function initScreenshotSession() {
  ...
  const sessionDir = path.join(screenshotDir, new Date().toISOString().replace(/:/g, '-'));
  ...
  return new Promise((resolve) => {
    const checkMidnight = () => {
      ...
      if (hours === 0 && minutes === 0 && seconds === 0) {
        ...
        log(`⏰ Waiting for midnight... Current CR time: ${crTime.toLocaleTimeString()}`, 'INFO');
...
async function reservePhase(page, courtConfig, targetDate, timeSlot) {
  try {
    log(`🎾 Phase 2: Reserving ${courtConfig.name} on ${targetDate.toDateString()} at ${timeSlot}`);
    ...
    if (dateStatus === 'not_found') {
      throw new Error(`DATE_NOT_AVAILABLE: Date ${targetDate.toDateString()} not found in calendar - may not be within booking window yet`);
    }
    ...
    if (dateStatus === 'not_clickable') {
      ...
      if (notAvailableMessage) {
        ...
        throw new Error(`DATE_NOT_CLICKABLE: Date ${targetDate.toDateString()} is in calendar but not clickable - may be fully booked`);
      }
    }
    ...
    if (!selected) {
      throw new Error(`TIME_SLOT_NOT_FOUND: Could not find time slot: ${timeSlot}`);
    }
    ...
    if (ARGS.watchMode) {
      log('WATCH MODE: Pausing 5 seconds before submission. Browser will stay open.', 'DEBUG');
      await new Promise(resolve => setTimeout(resolve, 5000));
    }
    ...
    for (const frame of page.frames()) {
      try {
        ...
        if (bodyText.includes('sobrepasado el limite') || bodyText.includes('límite permitido')) {
          errorMessage = 'RESERVATION_LIMIT: Reservation limit exceeded - you have already used your allowed reservations';
        } else if (bodyText.includes('excede la cantidad máxima de personas')) {
          errorMessage = 'SLOT_TAKEN: Time slot already taken - someone else reserved it first';
        } else if (bodyText.includes('ocupado') || bodyText.includes('no disponible')) {
          errorMessage = 'SLOT_OCCUPIED: Time slot is occupied or not available';
        }
      } catch (e) {
        // Skip frames we can't access
      }
    ...
    if (successFound) {
      ...
      throw new Error('UNKNOWN_ERROR: Could not confirm reservation success - no success message found');
    }
...
async function main() {
  ...
  log(`Mode: ${ARGS.test ? 'TEST' : 'PRODUCTION'}${ARGS.dryRun ? ' (DRY RUN)' : ''}${ARGS.debugMode ? ' (DEBUG)' : ''}`);
  ...
  try {
    ...
    const court1TimeSlot = ARGS.court1Time || CONFIG.courts.court1.slots[dayOfWeek1];
    const court2TimeSlot = ARGS.court2Time || CONFIG.courts.court2.slots[dayOfWeek2];
    ...
    if (ARGS.dryRun) {
      ...
      if (shouldReserveCourt1) log(`Would reserve: Court 1 on ${targetDateCourt1.toDateString()} at ${court1TimeSlot}`);
      if (shouldReserveCourt2) log(`Would reserve: Court 2 on ${targetDateCourt2.toDateString()} at ${court2TimeSlot}`);
      return;
    }
    ...
    if (!ARGS.test) {
      ...
      log(`⏰ TEST DELAY: Waiting ${delaySeconds} seconds before Phase 2 (simulating production timing)...`);
