...
export class MobileAPIClient {
  ...
  constructor(username: string, password: string, isPreHashed: boolean = false) {
    this.username = username;
    // MD5 hash the password (matches mobile app implementation)
    // If already hashed (32 char hex string), use directly
    if (isPreHashed || (password.length === 32 && /^[a-f0-9]+$/i.test(password))) {
  ...
  async createReservation(params: ReservationParams): Promise<ReservationResult> {
    ...
    console.log(`[API] Calling: area=${params.area}, day=${params.day}, schedule=${params.schedule}`);
